...
mod tests {
    use masonry::accesskit::{Node, Role};
    use masonry::core::{
        AccessCtx, ChildrenIds, LayoutCtx, MeasureCtx, NewWidget, NoAction, PaintCtx,
    ...
    /// Minimal container that can stash its child, so tests can put the spinner
    /// in the state a collapsed panel or a closed overlay would. Masonry's anim
    /// pass does not skip stashed widgets, so this is the only way to observe
    /// what a hidden spinner actually does.
    struct StashBox {
        child: WidgetPod<SpinnerWidget>,
    }
    ...
    impl Widget for StashBox {
        ...
        fn layout(&mut self, ctx: &mut LayoutCtx<'_>, _props: &PropertiesRef<'_>, size: Size) {
            if !ctx.child_is_stashed(&self.child) {
                ...
                ctx.place_child(&mut self.child, masonry::kurbo::Point::ZERO);
            }
        }
    ...
    fn stashable_harness() -> TestHarness<StashBox> {
        TestHarness::create(
            masonry::theme::default_property_set(),
            NewWidget::new(StashBox {
                child: WidgetPod::new(SpinnerWidget::new(Color::BLACK, 16.0)),
            }),
        )
    }
